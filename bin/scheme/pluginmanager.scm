...
(let ((table (<gui> :table (list "Usage" "Name" "Type" "Category" "Creator" "Path" "Inputs" "Outputs"))))
  (define use-x 0)
  (define name-x 1)
  (define type-x 2)
  (define category-x 3)
          ...
          (begin
            ...
            (when (or is-normal is-container)
              (let ((n (entry :num-uses)))
                (if (> n 0)
                    (<gui> :add-table-int-cell table n use-x y)
                    (<gui> :add-table-string-cell table "" use-x y)))
              (define name-gui (<gui> :add-table-string-cell table (entry :name) name-x y))
              (<gui> :add-table-string-cell table (entry :type-name) type-x y)
              (<gui> :add-table-string-cell table (entry :path) path-x y)
              (cond (is-normal
                     
                     (<gui> :add-table-string-cell table (<-> (entry :category)) category-x y)
                     (<gui> :add-table-string-cell table (<-> (entry :creator)) creator-x y)
                     (<gui> :add-table-int-cell table (entry :num-inputs) inputs-x y)
                     (<gui> :add-table-int-cell table (entry :num-outputs) outputs-x y))
                    ...
                    (is-container
                     ...
                     (<gui> :add-table-string-cell table "" category-x y)
                     (<gui> :add-table-string-cell table "" creator-x y)
                     (<gui> :add-table-gui-cell table pop1 inputs-x y)
                     (<gui> :add-table-gui-cell table pop2 outputs-x y)
                     )
                    ))
            (if (> (- (time) start-time)
                   0.1)
  ...
  (define (stop-search)
    (when curr-add-rows-callback
      (<ra> :remove-schedule curr-add-rows-callback)
      (set! curr-add-rows-callback #f)))

  (define (finished-searching?)
    (not curr-add-rows-callback)) ;; yepp. If searching, either the PC can not be here (there's just one thread), or curr-add-rows-callback has a value.
...
;      (<gui> :enable-table-sorting table #f)
  ...
  (<gui> :stretch-table table use-x #f (floor (* 1.5 (<gui> :text-width "Usage"))))
  (<gui> :stretch-table table name-x #f (floor (* 1.5 (<gui> :text-width "Multi-band compressor"))))
  (<gui> :stretch-table table type-x #f (floor (* 1.5 (<gui> :text-width "Sample Player"))))
  (<gui> :stretch-table table category-x #f (floor (* 1.5 (<gui> :text-width "Category"))))
  (<gui> :stretch-table table creator-x #f (floor (* 1.5 (<gui> :text-width "Joern Nettingsmeier"))))
  (<gui> :stretch-table table path-x #t 2)
  (<gui> :stretch-table table inputs-x #f  (floor (* 1.5 (<gui> :text-width "Outputs")))) ;; same width for input and output
  (<gui> :stretch-table table outputs-x #f (floor (* 1.5 (<gui> :text-width "Outputs"))))
