...
(define (set-left-interior-status-bar seqblocknum seqtracknum)
  (<ra> :set-statusbar-text (<-> "----|: " (two-decimal-string (/ (<ra> :get-seqblock-interior-start-gfx seqblocknum seqtracknum)
                                                                  (<ra> :get-sample-rate))))))
...
(define (get-editor-track-on-off-for-seqblock seqtracknum seqblocknum)
...
(define (get-min-seqblock-end-interior min-num-pixels seqblock-info)
  (define seqtracknum (seqblock-info :seqtracknum))
  (define seqblocknum (seqblock-info :seqblocknum))
  (define start-time (<ra> :get-seqblock-interior-start-gfx seqblocknum seqtracknum))
...
(define (get-max-seqblock-start-interior min-num-pixels seqblock-info)
  (define seqtracknum (seqblock-info :seqtracknum))
  (define seqblocknum (seqblock-info :seqblocknum))
  (define end-time (<ra> :get-seqblock-interior-end-gfx seqblocknum seqtracknum))
...
(define (get-seqblock-interior-pixels-per-value-unit seqblock-info)
  (define seqtracknum (seqblock-info :seqtracknum))
  (define seqblocknum (seqblock-info :seqblocknum))
  (let ((ret (* (<ra> :get-seqblock-stretch-gfx seqblocknum seqtracknum)
...
(define (get-max-seqblock-start-pos min-num-pixels seqblock-info)
  (define seqtracknum (seqblock-info :seqtracknum))
  (define seqblocknum (seqblock-info :seqblocknum))
  (define end-time (<ra> :get-seqblock-gfx-end-time seqblocknum seqtracknum))
...
(add-node-mouse-handler :Get-area-box (lambda()
                        ...
                        :Get-existing-node-info (lambda (X Y callback)
                                                  ...
                                                  (define seqtracknum (seqblock-info :seqtracknum))
                                                  (define seqblocknum (seqblock-info :seqblocknum))
                                                  (define start-pos (<ra> :get-seqblock-end-time seqblocknum seqtracknum))
                                                  (callback seqblock-info start-pos Y))

                        :Get-min-value (lambda (seqblock-info)
                                         (get-min-seqblock-end-pos *min-seqblock-width* seqblock-info)
                                         )
                        ...
                        :Get-max-value (lambda (seqblock-info)
                                         ...
                                         (if (or #t (= (1- num-seqblocks) seqblocknum))
                                             (+ 10000000000 (<ra> :get-seqblock-end-time (seqblock-info :seqblocknum) seqtracknum)) ;; always return this one. It's more accurate to limit size in common/seqtrack.c instead.
                        ...
                        :Move-node (lambda (seqblock-info Value Y)
                                     ...
                                     (define new-pos (if (<ra> :control-pressed)
                                                         (floor Value)
                                                         (<ra> :get-seq-gridded-time (floor Value) 0 (<ra> :get-seq-block-grid-type)))
                                       )
                        ...
                        :Publicize (lambda (seqblock-info)
                                     (define seqtracknum (seqblock-info :seqtracknum))
                                     (define seqblocknum (seqblock-info :seqblocknum))
                                     (<ra> :set-statusbar-text (<-> "Stretch: " (two-decimal-string (<ra> :get-seqblock-stretch-gfx seqblocknum seqtracknum)))))
...
(add-node-mouse-handler :Get-area-box (lambda()
                        ...
                        :Get-existing-node-info (lambda (X Y callback)
                                                  ...
                                                  (define seqtracknum (seqblock-info :seqtracknum))
                                                  (define seqblocknum (seqblock-info :seqblocknum))
                                                  (define start-pos (<ra> :get-seqblock-start-time seqblocknum seqtracknum))
                                                  (callback seqblock-info start-pos Y))

                        :Get-min-value (lambda (seqblock-info)
                                         0) ;; the API does a better job of limiting min value.
                        
                        :Get-max-value (lambda (seqblock-info)
                                         (get-max-seqblock-start-pos *min-seqblock-width* seqblock-info))
                        ...
                        :Move-node (lambda (seqblock-info Value Y)
                                     ...
                                     (define new-pos (if (or (and (= 0 seqtracknum)
                                                                  (= 0 seqblocknum))
                                                             (<ra> :control-pressed))
                                                         (floor Value)
                                                         (<ra> :get-seq-gridded-time (floor Value) 0 (<ra> :get-seq-block-grid-type)))
                                     ...
                                     (when (< new-pos (get-max-seqblock-start-pos *min-seqblock-width* seqblock-info))
                                       (set-grid-type #t)                                       
                                       (define end-time (<ra> :get-seqblock-gfx-end-time seqblocknum seqtracknum))
...
(define (move-seqblock-to-new-seqtrack seqtracknum new-seqtracknum seqblocknum new-pos add-undo)
  (ignore-undo-block (lambda ()
                       ...
                       (set! *current-seqblock-info* #f)
                       (define seqblock (copy-seqblock (create-seqblock-from-existing-seqblock seqtracknum seqblocknum)
                                        ...
                                        (if (not (= old-pos new-pos))
                                            (begin
                                              (when (not gakkgakk-has-made-undo)
                                                ...
                                                (set! gakkgakk-has-made-undo #f))
                                              (begin
                                                (<ra> :move-seqblock new-pos seqblocknum seqtracknum)
                                                ;;(c-display "moving 1")
                                                ))
                                            (begin
                                              (<ra> :move-seqblock-gfx old-pos seqblocknum seqtracknum)
                        ...
                        :Move-node (lambda (seqblock-info Value Y)
                                     ...
                                     (if (not (= (seqblock-info :seqtracknum) new-seqtracknum))
                                         ...
                                         (begin
                                           ...
                                           (move-seqblock-to-new-seqtrack seqtracknum new-seqtracknum seqblocknum new-pos (not gakkgakk-has-made-undo)))
                                         ...
                                         (begin
                                           ...
                                           (cond (#f #f)
                                                 ...
                                                 (else
                                                  ...
                                                  (<ra> :move-seqblock-gfx new-pos seqblocknum seqtracknum)
                                                  seqblock-info)))))
...
(define (get-nonstretched-seqblock-duration seqblocknum seqtracknum)
  (- (<ra> :get-seqblock-interior-end-gfx seqblocknum seqtracknum)
     (<ra> :get-seqblock-interior-start-gfx seqblocknum seqtracknum)))
                ...
                (and (= Button *right-button*)
                     ...
                     *current-seqblock-info*
                     (let ((seqtracknum (*current-seqblock-info* :seqtracknum))
                       ...
                       (and (or (inside-box (<ra> :get-box seqblock-right-stretch seqblocknum seqtracknum) X Y)
                                (inside-box (<ra> :get-box seqblock-left-stretch seqblocknum seqtracknum) X Y))
                            (begin
