  ...
  // background
  {
    ...
    
    {
      ...
      if (WSIGNATURE_is_first_beat(wsignature))
        c = GE_mix_color_z(GE_get_rgb(c15), Black_rgb(), first_beat_opacity, Z_BACKGROUND | Z_STATIC_X);
      else if (wsignature->beat_num>0)
        c = GE_mix_color_z(GE_get_rgb(c15), Black_rgb(), beat_opacity, Z_BACKGROUND | Z_STATIC_X);
      else {
        if(localzoom->level>0 && localzoom->zoomline>0 && localzoom->autogenerated==false)
          c = GE_mix_color_z(GE_get_rgb(c15), White_rgb(), 925, Z_BACKGROUND | Z_STATIC_X); // manually splitted line
        else
          c = GE_color_z(c15, Z_BACKGROUND | Z_STATIC_X);
      }
    ...
    if (g_colored_tracks) {
      ...
      while(wtrack!=NULL && wtrack->l.num<=wblock->right_track){
        ...
        if (patch != NULL){
          ...
          if (WSIGNATURE_is_first_beat(wsignature))
            c = GE_mix_color_z(GE_get_custom_rgb(colornum), Black_rgb(), first_beat_opacity, Z_BACKGROUND | Z_STATIC_X);
          else if (wsignature->beat_num>0)
            c = GE_mix_color_z(GE_get_custom_rgb(colornum), Black_rgb(), beat_opacity, Z_BACKGROUND | Z_STATIC_X);
          else {
            if(localzoom->level>0 && localzoom->zoomline>0 && localzoom->autogenerated==false)
              c = GE_mix_color_z(GE_get_custom_rgb(colornum), White_rgb(), 925, Z_BACKGROUND | Z_STATIC_X); // manually splitted line
            else
              c = GE_z(GE_get_custom_rgb(colornum), Z_BACKGROUND | Z_STATIC_X);
          }
...
static void create_track_text(const struct Tracker_Windows *window, const struct WBlocks *wblock, const struct WTracks *wtrack, vector_t *tr, int realline){
  ...
  enum ColorNums colnum = get_colnum(tr2, isranged);
  
  if (isranged)
    GE_filledBox(GE_color(RANGE_COLOR_NUM),wtrack->notearea.x,y1,wtrack->notearea.x2,y2);
