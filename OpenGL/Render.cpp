...
static void draw_linenumber(const struct Tracker_Windows *window, const struct WBlocks *wblock, int colornum, int linenumber, int realline, bool is_zoomline){
  ...
  draw_text_num(
                window,
                c,
                is_zoomline ? linenumber : 1 + linenumber/(root->lpb*4),
                //linenumber,
                width,
                x,
                y
...
static void draw_realline_linenumber(const struct Tracker_Windows *window, const struct WBlocks *wblock, int realline){
  ...
  if(localzoom->level>0){
    if(localzoom->zoomline>0){
      if (localzoom->autogenerated==true)
        colornum=-13;
      else
        colornum=13; //R_MIN(7,localzoom->level+1);
    }else{
  ...
  if(localzoom->level>0 && localzoom->zoomline>0){
    if (true || localzoom->zoomline>1
        || true // (realline+1 < wblock->num_reallines && wblock->reallines[realline+1]->zoomline==localzoom->zoomline+1)  // only show subline number if there are more than one subline
        )
...
void create_block_borders(
  ...
  */
  ...
  create_single_border(
                       wblock->linenumarea.x+window->fontwidth*3-1,
                       y1,y2
                       );
