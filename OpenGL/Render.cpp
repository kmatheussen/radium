...
/************************************
   swing track (main swing track)
 ************************************/

static void create_swing(const struct Tracker_Windows *window, const struct WBlocks *wblock, int realline, int weight, enum WSwingType type, int logtype, bool autogenerated){
  int y    = get_realline_y1(window, realline);
  ...
  if(type!=WSWING_NORMAL){
    ...
    GE_text(GE_color_alpha_z(TEXT_COLOR_NUM, 0.3, Z_ZERO, y), typetext, wblock->swingTypearea.x, y);
  }

  if (logtype==LOGTYPE_HOLD){
    GE_text(c, "|", wblock->swingarea.x + window->fontwidth*2, y);
  }
}


static void create_swingtrack(const struct Tracker_Windows *window, const struct WBlocks *wblock){
  ...
  for(int i = 0; i < barswings->num_elements ; i++){
    ...
    for(int swingnum = 0 ; swingnum < swings->num_elements ; swingnum++){
      ...
      if (p_Greater_Or_Equal(place, realline_place2)){
        ...
        if (curr_weight != -1)
          create_swing(window, wblock, realline, curr_weight, type, curr_logtype, curr_autogenerated);
      ...
      curr_autogenerated = autogenerated;
...
static void GL_create2(const struct Tracker_Windows *window, struct WBlocks *wblock){
  ...
  GE_start_writing(y2); {
    ...
    if(window->show_swing_track)
      create_swingtrack(window, wblock);
    if(window->show_lpb_track)
