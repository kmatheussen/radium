...
{
  GE_line(GE_color_alpha(7,0.5),x,y,x,y2,0.5);
}
...
static void create_background_realline(const struct Tracker_Windows *window, const struct WBlocks *wblock, const struct WSignatures *wsignature, int realline){
  ...
  // background
  {
    ...
    if (WSIGNATURE_is_first_beat(wsignature))
      c = GE_mix_color_z(GE_get_rgb(15), Black_rgb(), first_beat_opacity, Z_BACKGROUND | Z_STATIC_X);
    else if (wsignature->beat_num>0)
      c = GE_mix_color_z(GE_get_rgb(15), Black_rgb(), beat_opacity, Z_BACKGROUND | Z_STATIC_X);
    else {
      if(localzoom->level>0 && localzoom->zoomline>0 && localzoom->autogenerated==false)
        c = GE_mix_color_z(GE_get_rgb(15), White_rgb(), 925, Z_BACKGROUND | Z_STATIC_X); // manually splitted line
      else
  ...
  if(1){
    ...
    if(line_opacity != 1000) {
      ...
      if (WSIGNATURE_is_first_beat(wsignature))
        c = GE_mix_color_z(GE_get_rgb(15), Black_rgb(), first_beat_opacity-250, Z_ABOVE(Z_BACKGROUND) | Z_STATIC_X);
      else if (wsignature->beat_num>0)
        c = GE_mix_color_z(GE_get_rgb(15), Black_rgb(), beat_opacity-250, Z_ABOVE(Z_BACKGROUND) | Z_STATIC_X);
      else
        c = GE_mix_color_z(GE_get_rgb(15), Black_rgb(), line_opacity, Z_ABOVE(Z_BACKGROUND) | Z_STATIC_X);
...
static void draw_linenumber(const struct Tracker_Windows *window, const struct WBlocks *wblock, const struct WSignatures *wsignature, int realline){
  ...
  if (linenumbersVisible() && localzoom->l.p.counter==0) {
    ...
    draw_text_num(
                  window,
                  GE_color_alpha_z(1, 0.45, Z_ZERO),
                  wsignature->beat_num,
...
static GE_Rgb get_note_color(float notenum){
  ...
  if(notenum<split1)
    rgb = GE_mix(GE_get_rgb(5), Black_rgb(), scale(notenum,0,split1,0,1000));
  else if(notenum<split2)
    rgb = GE_mix(GE_get_rgb(6), GE_get_rgb(5), scale(notenum,split1,split2,0,1000));
  else
    rgb = GE_mix(White_rgb(), GE_get_rgb(6), scale(notenum,split2,160,0,1000));
...
static void create_track_peaks(const struct Tracker_Windows *window, const struct WBlocks *wblock, const struct WTracks *wtrack, const struct Notes *note, const struct NodeLine *nodelines){
  ...
  GE_Context *c = GE_mix_color_z(GE_get_rgb(0), GE_get_rgb(2), 500, Z_ZERO);
