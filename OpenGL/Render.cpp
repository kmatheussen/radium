...
static GE_Rgb shade_realline(const GE_Rgb rgb, const struct WSignatures *wsignature, const struct LocalZooms *localzoom){
  if (WSIGNATURE_is_first_beat(wsignature))
    return GE_mix(rgb, Black_rgb(), first_beat_opacity);
  
  else if (wsignature->beat_num>0)
    return GE_mix(rgb, Black_rgb(), beat_opacity);
  
  else if(localzoom->level>0 && localzoom->zoomline>0 && localzoom->autogenerated==false)
    return GE_mix(rgb, White_rgb(), 925); // manually splitted line
...
static void create_background_realline(const struct Tracker_Windows *window, const struct WBlocks *wblock, const struct WSignatures *wsignature, int realline){
  ...
  // background
  {
    ...
    {
      GE_Context *c = GE_z(shade_realline(GE_get_rgb(c15), wsignature, localzoom), Z_BACKGROUND | Z_STATIC_X);
