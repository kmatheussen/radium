...
static void draw_node_indicator(float x,
                                float y)
{  
  GE_Context *c = GE_color_alpha_z(AUTOMATION_INDICATOR_COLOR_NUM, 0.6, Z_MAX_SCROLLTRANSFORM);
                            ...
                            )
{
  ...
  if (node == current_node) {
    GE_filledBox(GE_mix_alpha_z(GE_get_rgb(color), White_rgb(), 300, 0.3, z),
                 x1,y1,
                 x2-1,y2
                 );
  }
  ...
  // vertical left
  GE_line(GE_mix_alpha_z(GE_get_rgb(color), White_rgb(), 100, 0.3, z),
          x1+1, y1+1,
          x1+2,y2-1,
          width);

  // horizontal bottom
  GE_line(GE_mix_alpha_z(GE_get_rgb(color), Black_rgb(), 300, 0.3, z),
          x1+2,y2-1,
          x2-1,y2-2,
          width);

  // vertical right
  GE_line(GE_mix_alpha_z(GE_get_rgb(color), Black_rgb(), 400, 0.3, z),
          x2-1,y2-2,
          x2-2,y1+2,
          width);

  // horizontal top
  GE_line(GE_mix_alpha_z(GE_get_rgb(color), White_rgb(), 300, 0.3, z),
          x2-2,y1+2,
          x1+1,y1+1,
          width);
}
...
{
  GE_line(Black_color(),x,y,x,y2,0.5);
  GE_line(GE_color(TRACK_SEPARATOR2_COLOR_NUM),x+1,y,x+1,y2,0.5);
}
...
{
  GE_line(GE_color(TRACK_SEPARATOR1_COLOR_NUM),x,y,x,y2,0.5);
}
...
{
  GE_line(GE_color_alpha(TRACK_SEPARATOR1_COLOR_NUM,0.5),x,y,x,y2,0.5);
}
...
static GE_Rgb shade_realline(const GE_Rgb rgb, bool is_current_track, const struct WSignatures *wsignature, const struct LocalZooms *localzoom){
  const GE_Rgb rgb2 =
    is_current_track
...
static void create_background_realline(const struct Tracker_Windows *window, const struct WBlocks *wblock, const struct WSignatures *wsignature, int realline){
  ...
  // background
  {
    ...
    {
      GE_Context *c = GE_z(shade_realline(GE_get_rgb(c15), false, wsignature, localzoom), Z_BACKGROUND | Z_STATIC_X);
  ...
  if(1){
    ...
    if(line_opacity != 1000) {
      ...
      if (WSIGNATURE_is_first_beat(wsignature))
        c = GE_mix_color_z(GE_get_rgb(c15), Black_rgb(), first_beat_opacity-250, Z_ABOVE(Z_BACKGROUND) | Z_STATIC_X);
      else if (wsignature->beat_num>0)
        c = GE_mix_color_z(GE_get_rgb(c15), Black_rgb(), beat_opacity-250, Z_ABOVE(Z_BACKGROUND) | Z_STATIC_X);
      else
        c = GE_mix_color_z(GE_get_rgb(c15), Black_rgb(), line_opacity, Z_ABOVE(Z_BACKGROUND) | Z_STATIC_X);
...
static void draw_linenumber(const struct Tracker_Windows *window, const struct WBlocks *wblock, const struct WSignatures *wsignature, int realline){
  ...
  if (linenumbersVisible() && localzoom->l.p.counter==0) {
    ...
    draw_text_num(
                  window,
                  GE_textcolor_z(TEXT_COLOR_NUM, z),
                  localzoom->l.p.line,
    ...
    draw_text_num(
                  window,
                  GE_textcolor_z(BAR_TEXT_COLOR_NUM, z),
                  wsignature->bar_num,
    ...
    draw_text_num(
                  window,
                  GE_color_alpha_z(TEXT_COLOR_NUM, 0.45, Z_ZERO),
                  wsignature->beat_num,
...
static void create_linenumbers(const struct Tracker_Windows *window, const struct WBlocks *wblock, const struct WSignatures *wsignatures){
  int realline;
  for(realline = 0 ; realline<wblock->num_reallines ; realline++)
...
static void create_tempograph(const struct Tracker_Windows *window, const struct WBlocks *wblock){
  ...
  float width = 1.3;
  GE_Context *c = GE_color(TEMPOGRAPH_COLOR_NUM);
...
static void create_signature(const struct Tracker_Windows *window, const struct WBlocks *wblock, const struct WSignatures *wsignatures, int realline, int x2){
  ...
  if (WSIGNATURE_is_measure_change(&wsignatures[realline])) {
    ...
    GE_text(GE_textcolor_z(TEXT_COLOR_NUM, Z_ZERO),
            temp,
            x,
            y
            );
...
static void create_lpb(const struct Tracker_Windows *window, const struct WBlocks *wblock, struct WLPBs *wlpbs, int realline){
  ...
  if(lpb!=0){
    draw_text_num(
                  window,
                  GE_textcolor_z(TEXT_COLOR_NUM, Z_ZERO),
                  lpb,
  ...
  if(type!=LPB_NORMAL){
    ...
    GE_text(GE_color_alpha_z(TEXT_COLOR_NUM, 0.3, Z_ZERO), typetext, wblock->lpbTypearea.x, y);
  }
}
...
static void create_bpm(const struct Tracker_Windows *window, const struct WBlocks *wblock, const struct WBPMs *wbpms, int realline){
  ...
  if(type!=TEMPO_NORMAL){
    ...
    GE_text(GE_color_alpha_z(TEXT_COLOR_NUM, 0.3, Z_ZERO), typetext, wblock->tempoTypearea.x, y);
  }
}
...
static void paint_tr2(TrackRealline2 *tr2, char **NotesTexts, int num, int x, int y){
  GE_Context *c = GE_textcolor(get_colnum(tr2));
...
static void paint_multinotes(const struct WTracks *wtrack, vector_t *tr, char **NotesTexts, int y1, int y2){
  int num_elements = tr->num_elements;

  int x1 = wtrack->notearea.x;
  int y_middle = (y1+y2)/2;

  paint_tr2((TrackRealline2*)tr->elements[0], NotesTexts, 1, x1, y1);
  
  paint_tr2((TrackRealline2*)tr->elements[1], NotesTexts, 2, x1, y_middle);
  ...
  paint_tr2((TrackRealline2*)tr->elements[2], NotesTexts, 3, x_middle, y1);
  
  if (num_elements == 3)
    return;

  if (num_elements>4){
    GE_Context *c = GE_textcolor(get_colnum(NULL));
    paint_halfsize_note(c, 4, NotesTexts[NOTE_MUL], x_middle, y_middle);
  } else {
    paint_tr2((TrackRealline2*)tr->elements[3], NotesTexts, 4, x_middle, y_middle);
  }
}
...
static void create_pianoroll_grid(const struct Tracker_Windows *window, const struct WBlocks *wblock, const struct WTracks *wtrack){
  ...
  GE_Context *octave_color = GE_color_alpha(PIANOROLL_OCTAVE_COLOR_NUM, 0.25);
  //GE_Context *note_color = GE_color_alpha(8, 0.5);

  GE_Context *white_key_color = GE_color_alpha(WHITE_COLOR_NUM, 0.1);
  GE_Context *black_key_color = GE_color_alpha(BLACK_COLOR_NUM, 0.1);
...
static void create_pianoroll(const struct Tracker_Windows *window, const struct WBlocks *wblock, const struct WTracks *wtrack){
  ...
  if (wtrack->track->patch==NULL)
    ...
    note_color = GE_mix_color(GE_get_custom_rgb(wtrack->track->patch->colornum), GE_get_rgb(colornum), 400);
...
static void create_track_is_recording(const struct Tracker_Windows *window, const struct WBlocks *wblock, const struct WTracks *wtrack){
  GE_Context *c = GE_z(GE_alpha(GE_get_rgb(RED_COLOR_NUM), 0.7), Z_STATIC);
...
static void create_range(const struct Tracker_Windows *window, const struct WBlocks *wblock){
  ...
  GE_Context *c = GE_color_alpha_z(RANGE_COLOR_NUM, 0.6, Z_MAX_SCROLLTRANSFORM);
...
static void create_cursor(const struct Tracker_Windows *window, const struct WBlocks *wblock){
  ...
  c = GE_z(GE_alpha(Black_rgb(), 0.75), Z_STATIC);
  ...
  c = GE_z(GE_alpha(White_rgb(), 0.05), Z_STATIC);
  GE_filledBox(c, 
               x2, y1,
               x3, y2-1
               );
...
void GL_create(const struct Tracker_Windows *window, struct WBlocks *wblock){
  ...
  GE_start_writing(); {
    ...
    create_linenumbers(window, wblock, wsignatures);
