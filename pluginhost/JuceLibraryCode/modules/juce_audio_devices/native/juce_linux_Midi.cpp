...
Array<MidiDeviceInfo> MidiOutput::getAvailableDevices()
{
    ...
    for (auto devicename : getDevices()){
      MidiDeviceInfo device (devicename, devicename);
      devices.add(device);
    }
...
std::unique_ptr<MidiOutput> MidiOutput::openDevice (int deviceIndex)
{
    StringArray devices;
    ...
    std::unique_ptr<MidiOutput> newDevice (new MidiOutput (devices [deviceIndex], devices [deviceIndex]));
    port->setupOutput();
...
std::unique_ptr<MidiOutput> MidiOutput::createNewDevice (const String& deviceName)
{
    ...
    std::unique_ptr<MidiOutput> newDevice (new MidiOutput (deviceName, deviceName));
    port->setupOutput();
    newDevice->internal = port;
...
std::unique_ptr<MidiInput> MidiInput::openDevice (int deviceIndex, MidiInputCallback* callback)
{
    StringArray devices;
    ...
    std::unique_ptr<MidiInput> newDevice (new MidiInput (devices [deviceIndex], devices [deviceIndex]));
    port->setupInput (newDevice.get(), callback);
    newDevice->internal = port;
    return newDevice;
}

std::unique_ptr<MidiInput> MidiInput::openDevice (const String&, MidiInputCallback*)      { abort(); return {}; }
...
std::unique_ptr<MidiInput> MidiInput::createNewDevice (const String& deviceName, MidiInputCallback* callback)
{
    ...
    std::unique_ptr<MidiInput> newDevice (new MidiInput (deviceName, deviceName));
    port->setupInput (newDevice.get(), callback);
    newDevice->internal = port;
    return newDevice;
}
