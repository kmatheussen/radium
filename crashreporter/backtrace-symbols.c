/*
Downloaded from http://opensource.apple.com/source/X11libs/X11libs-40.2/cairo/cairo-1.8.6/util/backtrace-symbols.c?txt
*/
...
static int find_matching_file(struct dl_phdr_info *info, size_t size, void *data)
{
	...
	for (n = info->dlpi_phnum; --n >= 0; phdr++) {
		if (phdr->p_type == PT_LOAD) {
			...
			if ((size_t)match->address >= vaddr && (size_t)match->address < vaddr + phdr->p_memsz) {
				/* we found a match */
