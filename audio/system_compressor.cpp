...
Compilation options: -lang cpp -ct 1 -cn Faust_system_compressor -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0 -vec -lv 0 -vs 32
------------------------------------------------------------ */
...
class Faust_system_compressor : public dsp {
 ...
 public:
	Faust_system_compressor() {
	}
	
	void metadata(Meta* m) { 
		m->declare("compile_options", "-lang cpp -ct 1 -cn Faust_system_compressor -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0 -vec -lv 0 -vs 32");
	...
	virtual void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->addHorizontalSlider("Ratio", &fHslider3, FAUSTFLOAT(2.0f), FAUSTFLOAT(1.0f), FAUSTFLOAT(2e+01f), FAUSTFLOAT(0.1f));
		...
		ui_interface->addHorizontalSlider("Threshold", &fHslider4, FAUSTFLOAT(-2e+01f), FAUSTFLOAT(-2e+01f), FAUSTFLOAT(2e+01f), FAUSTFLOAT(0.1f));
		...
		ui_interface->addHorizontalSlider("Attack", &fHslider2, FAUSTFLOAT(50.148f), FAUSTFLOAT(0.0f), FAUSTFLOAT(5e+02f), FAUSTFLOAT(0.1f));
		...
		ui_interface->addHorizontalSlider("Release", &fHslider1, FAUSTFLOAT(100.237f), FAUSTFLOAT(0.0f), FAUSTFLOAT(1e+03f), FAUSTFLOAT(0.1f));
		...
		ui_interface->addHorizontalSlider("Output Gain", &fHslider0, FAUSTFLOAT(0.0f), FAUSTFLOAT(-4e+01f), FAUSTFLOAT(4e+01f), FAUSTFLOAT(0.1f));
		ui_interface->declare(&fHbargraph0, "0", "");
		ui_interface->declare(&fHbargraph0, "7", "");
		ui_interface->addHorizontalBargraph("Gakk", &fHbargraph0, FAUSTFLOAT(-5e+01f), FAUSTFLOAT(1e+01f));
		ui_interface->declare(&fHbargraph1, "1", "");
		ui_interface->declare(&fHbargraph1, "7", "");
		ui_interface->addHorizontalBargraph("Gakk", &fHbargraph1, FAUSTFLOAT(-5e+01f), FAUSTFLOAT(1e+01f));
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		float fZec0[32];
		float fSlow1 = std::max<float>(fConst1, 0.001f * float(fHslider1));
		float fSlow2 = ((fSlow1 > 0.0f) ? std::exp(-(fConst1 / fSlow1)) : 0.0f);
		float fSlow3 = 1.0f - fSlow2;
		float fRec3_tmp[36];
		float* fRec3 = &fRec3_tmp[4];
		float fSlow4 = std::max<float>(fConst1, 0.001f * float(fHslider2));
		float fSlow5 = ((fSlow4 > 0.0f) ? std::exp(-(fConst1 / fSlow4)) : 0.0f);
		float fSlow6 = 1.0f - fSlow5;
		float fRec2_tmp[36];
		float* fRec2 = &fRec2_tmp[4];
		float fSlow7 = (((0.5f * fSlow4) > 0.0f) ? std::exp(-(fConst2 / fSlow4)) : 0.0f);
		...
		for (vindex = 0; vindex <= (count - 32); vindex = vindex + 32) {
			...
			/* Compute code */
			for (int i = 0; i < vsize; i = i + 1) {
				fZec1[i] = 8.685889f * (8.262958e-08f * float(pun_float_to_int(fRec2[i])) + -87.98997f);
			}
			/* Vectorizable loop 4 */
			/* Compute code */
			for (int i = 0; i < vsize; i = i + 1) {
			...
			/* Compute code */
			for (int i = 0; i < vsize; i = i + 1) {
				fZec3[i] = pun_int_to_float(int(8388608.0f * (std::max<float>(-126.0f, 0.1660964f * fRec1[i]) + 126.942696f)));
			}
			/* Vectorizable loop 8 */
			/* Compute code */
			for (int i = 0; i < vsize; i = i + 1) {
