...
static bool get_blub_effects_state(const struct Patch *patch, const SoundPlugin *plugin, const hash_t *state, blub_t *&blub){
  ...
  //
  {
    ...
    if (num_effects_in_new_state != num_effects){
      ...
      if (num_effects_in_new_state > 0){
        addMessage(talloc_format("Old AB state is not compatible with current plugin (%d vs. %d). Can not apply effects for \"%s\".", num_effects_in_new_state, num_effects, patch->name));
        if (!strcmp(RADIUM_VERSION, "6.5.78"))
          addMessage("It might be possible to fix this by loading and saving your song in Radium V6.5.78.\n");
        else
          addMessage("\n");
      }
