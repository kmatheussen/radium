...
static bool has_recursive_event_connection(SoundPlugin *plugin, SoundPlugin *start_plugin){
  if(start_plugin==plugin)
    return true;

  int num_event_receivers = plugin->num_event_receivers;
  int i;
  for(i=0; i<num_event_receivers; i++)
    if(has_recursive_event_connection(plugin->event_receivers[i], start_plugin)==true)
      return true;
...
bool PLUGIN_add_event_receiver(SoundPlugin *source, SoundPlugin *destination){
...
void PLUGIN_remove_event_receiver(SoundPlugin *source, SoundPlugin *destination){
  int i;
  int num_event_receivers = source->num_event_receivers;
...
void PLUGIN_remove_all_event_receivers(SoundPlugin *plugin){
  int num_event_receivers = plugin->num_event_receivers;

  PLAYER_lock();{
    int i;
    for(i=0; i<num_event_receivers; i++)
      plugin->event_receivers[i] = NULL;

    plugin->num_event_receivers = 0;
  }PLAYER_unlock();
}
