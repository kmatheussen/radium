       ...
       libpd_is_float(argv[5]))
      {
        ...
        if (patch!=NULL) {
          RT_PLAYER_runner_lock();{
            ...
            RT_PATCH_send_play_note_to_receivers(seqtrack, patch, create_note_t(NULL, note_id, pitch, velocity, pan, 0), time);
          }RT_PLAYER_runner_unlock();
        }
      }
    else
    ...
    if(argc==5 &&
       ...
       libpd_is_float(argv[4]))
      {
        ...
        if (patch!=NULL) {
          RT_PLAYER_runner_lock();{
            struct SeqTrack *seqtrack = RT_get_curr_seqtrack();
            RT_PATCH_send_change_velocity_to_receivers(seqtrack, patch, create_note_t(NULL, note_id, notenum, velocity, 0, 0), time);
          }RT_PLAYER_runner_unlock();
        }
      }
    else
    ...
    if(argc==5 &&
       ...
       libpd_is_float(argv[4]))
      {
        ...
        if (patch!=NULL) {
          RT_PLAYER_runner_lock();{
            struct SeqTrack *seqtrack = RT_get_curr_seqtrack();
            RT_PATCH_send_change_pitch_to_receivers(seqtrack, patch, create_note_t(NULL, note_id, notenum, 0, pitch, 0), time);
          }RT_PLAYER_runner_unlock();
        }
      }
    else
...
static void RT_noteonhook(void *d, int channel, int pitch, int velocity){
  ...
  RT_PLAYER_runner_lock();{
    struct SeqTrack *seqtrack = RT_get_curr_seqtrack();
    if(velocity>0)
      RT_PATCH_send_play_note_to_receivers(seqtrack, (struct Patch*)patch, create_note_t(NULL, -1, pitch, (float)velocity / 127.0f, 0.0f, channel), -1);
    else
      RT_PATCH_send_stop_note_to_receivers(seqtrack, (struct Patch*)patch, create_note_t(NULL, -1, pitch, 0, 0, channel), -1);
  }RT_PLAYER_runner_unlock();
...
static void RT_polyaftertouchhook(void *d, int channel, int pitch, int velocity){
  ...
  RT_PLAYER_runner_lock();{
    struct SeqTrack *seqtrack = RT_get_curr_seqtrack();
    RT_PATCH_send_change_velocity_to_receivers(seqtrack, (struct Patch*)patch, create_note_t(NULL, -1, pitch, (float)velocity / 127.0f, 0, channel), -1);
  }RT_PLAYER_runner_unlock();
