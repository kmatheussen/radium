...
class Brass_dsp : public dsp {
  ...
  public:
	...
	virtual void buildUserInterface(UI* interface) {
		...
		interface->declare(&fslider3, "tooltip", "Envelope release duration");
		interface->declare(&fslider3, "unit", "s");
		interface->addHorizontalSlider("Envelope_Release", &fslider3, 0.07f, 0.0f, 2.0f, 0.01f);
		interface->closeBox();
		...
		interface->addHorizontalSlider("Slide_Length", &fslider1, 0.041f, 0.01f, 1.0f, 0.001f);
		interface->closeBox();
		interface->closeBox();
	...
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		...
		for (int i=0; i<count; i++) {
			...
			float fTemp18 = ((fTemp16 * fRec17[1]) + (fTemp17 * fTemp14));
			...
			fRec16[0] = ((fTemp17 * fTemp15) + (fTemp14 * fRec17[1]));
