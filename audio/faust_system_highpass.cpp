...
class System_Highpass_dsp : public dsp {
  ...
  public:
	...
	virtual void buildUserInterface(UI* interface) {
		...
		interface->addVerticalSlider("Hp. Freq", &fVslider0, 315.0f, 4e+01f, 2e+04f, 1.0f);
		interface->closeBox();
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		for (int i = 0; (i < count); i = (i + 1)) {
			...
			float fTemp1 = (1.0f + (fRec0[0] * (fRec0[0] + 1.4142135f)));
			fRec1[0] = (float(input0[i]) - ((((1.0f + (fRec0[0] * (fRec0[0] - 1.4142135f))) * fRec1[2]) + (2.0f * ((1.0f - fTemp0) * fRec1[1]))) / fTemp1));
			output0[i] = FAUSTFLOAT(((((fTemp0 * fRec1[0]) + (2.0f * (fRec1[1] * (0.0f - fTemp0)))) + (fTemp0 * fRec1[2])) / fTemp1));
			fRec0[1] = fRec0[0];
			fRec1[2] = fRec1[1];
			fRec1[1] = fRec1[0];
