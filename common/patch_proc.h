...
extern LANGSPEC struct Patch *PATCH_get_current_audio(void); // Similar to PATCH_get_current(), but won't return a MIDI instrument.
extern LANGSPEC struct Patch *PATCH_get_current(void); // Can only return NULL during startup.
extern LANGSPEC struct Patch *RT_PATCH_get_current(void); // If called from an audio thread, we must call this function instead of PATCH_get_current.
...
extern LANGSPEC void PATCH_replace_permanent(struct Patch *old_patch, struct Patch *new_patch);
...
extern LANGSPEC void FX_call_me_before_starting_to_play_song2(struct SeqTrack *seqtrack, const struct SeqBlock *seqblock, STime start_time);
