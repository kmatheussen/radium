...
bool BPMTEXT_keypress(struct Tracker_Windows *window, struct WBlocks *wblock, int realline, Place *place, int key){
  ...
  if (false && bpms.size()  > 1) {
    ...
    data_as_text_t dat = DAT_get_newvalue(subsubtrack, key, root->tempo, 1, 999, 1, 999, false, false, false);

    if (dat.is_valid==false)
      return false;

    ADD_UNDO(Tempos_CurrPos(window));
    SetTempo(wblock->block, place, dat.value);
    ...
    if (key == EVENT_DEL) {
      ...
      data_as_text_t dat = DAT_get_overwrite(bpm->tempo, LOGTYPE_HOLD, subsubtrack, key, 1, 999, 1, 999, false, false);

      if (dat.is_valid==false)
        return false;

      SetTempo(wblock->block, &bpm->l.p, dat.value);
